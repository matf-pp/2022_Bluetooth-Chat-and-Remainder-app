...
class BluetoothChatService(context: Context, handler: Handler){
    ...
    private val MY_UUID_SECURE = UUID.fromString("f98de262-0cda-4827-97f3-023efd9d29c6")
     ...
     */
    @Synchronized fun connect(context1:Context,device: BluetoothDevice?, secure: Boolean) {
    ...
    @Synchronized fun connected(socket: BluetoothSocket?, device: BluetoothDevice?, socketType: String) {
        ...
        val msg1 = mHandler?.obtainMessage(Constants.MESSAGE_DEVICE_NAME) ////////////ne
        val bundle1 = Bundle()
        bundle1.putString(Constants.DEVICE_NAME, device?.name)/////////////////
        msg1?.data = bundle1
    ...
    private inner class AcceptThread(secure: Boolean) : Thread() {
        ...
        init {
            ...
            val msg = mHandler?.obtainMessage(Constants.MESSAGE_STATE_CHANGE)
            msg?.arg1= STATE_LISTEN
            val bundle = Bundle()
            bundle.putString(Constants.TOAST, "Device connection was lost")
            msg?.data = bundle
            mHandler?.sendMessage(msg!!)
    ...
    private inner class ConnectThread(private val mmDevice: BluetoothDevice?, secure: Boolean) : Thread() {
        ...
        override fun run() {
            ...
            try {
                ...
                mmSocket?.connect() ///////////////OVDEEEEEEEEEEEEEEEEEEEE
